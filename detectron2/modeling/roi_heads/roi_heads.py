...
def _perm_box(boxes, iou_thr=0.97, perm_range=0.01, counter=0):
    ...
    if (new_wh <= 0).any():
        is_valid = False
    iou = torchvision.ops.box_iou(ori_boxes.unique(dim=0), boxes)
    if (iou < iou_thr).any():
        is_valid = False
    # print(f'ori box: {ori_boxes.unique(dim=0)}')
    # print(f'new box: {boxes}')
    # print(f'iou: {iou}')
    if not is_valid:
        return _perm_box(
            ori_boxes,
            perm_range=perm_range - counter * 0.001,
            counter=counter + 1)
    return boxes


def perm_boxes(boxes):
    # boxes (tensor): [n, 4]
    ...
    for box in unique_boxes:
        ...
        if inds.float().sum().item() == 1:
            continue
        boxes[inds] = _perm_box(boxes[inds], counter=0)
...
class ROIHeads(torch.nn.Module):
    ...
    def label_and_sample_proposals(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            # only change
            tmp_boxes = copy.deepcopy(targets_per_image.gt_boxes)
            tmp_boxes.tensor = perm_boxes(tmp_boxes.tensor)
            match_quality_matrix = pairwise_iou(
                tmp_boxes, proposals_per_image.proposal_boxes
            )
