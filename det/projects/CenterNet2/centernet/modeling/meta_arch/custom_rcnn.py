...
class CustomRCNN(GeneralizedRCNN):
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return CustomRCNN._postprocess(results, batched_inputs,
                                           images.image_sizes)
        else:
            return results
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.dynamic_classifier and ann_type != 'caption':
            cls_inds = self._sample_cls_inds(gt_instances,
                                             ann_type)  # inds, inv_inds
            ind_with_bg = cls_inds[0].tolist() + [-1]
            cls_features = self.roi_heads.box_predictor[
                0].cls_score.zs_weight[:, ind_with_bg].permute(1,
                                                               0).contiguous()
    ...
    def _sample_cls_inds(self, gt_instances, ann_type='box'):
        ...
        inds = get_fed_loss_inds(gt_classes,
                                 self.num_sample_cats,
                                 C,
                                 weight=freq_weight)
