...
from .v3det import register_v3det_instances
...
def register_all_v3det(root):
    for dataset_name, splits_per_dataset in _PREDEFINED_SPLITS_V3DET.items():
        for key, (image_root, json_file) in splits_per_dataset.items():
            register_v3det_instances(
                key,
                os.path.join(root, json_file) if "://" not in json_file else json_file,
                os.path.join(root, image_root),
            )
