...
def fast_rcnn_inference(
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, image_shape, score_thresh, topk_before_nms, nms_thresh, topk_per_image,
...
def fast_rcnn_inference_single_image(
    ...
    if False: #topk_before_nms > 0 and len(scores) >= topk_before_nms*2:
        assert topk_before_nms >= topk_per_image
        pre_topk = scores.topk(topk_before_nms).indices
        keep = batched_nms(boxes[pre_topk], scores[pre_topk], filter_inds[pre_topk, 1], nms_thresh)
        pre_topk = pre_topk[keep]
        if len(pre_topk) < topk_per_image:
            pre_topk = torch.cat((pre_topk, scores.topk(topk_before_nms * 2).indices[topk_before_nms:topk_before_nms*2]), dim=0)
            keep = batched_nms(boxes[pre_topk], scores[pre_topk], filter_inds[pre_topk, 1], nms_thresh)
            pre_topk = pre_topk[keep]
        keep = pre_topk[:topk_per_image] if topk_per_image >= 0 else pre_topk
    else:
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            test_topk_before_nms(int): select topk bbox for nms, -1 disable
        ...
        self.test_topk_before_nms = test_topk_before_nms
    ...
    def losses(self, predictions, proposals):
        ...
        if self.use_sigmoid_ce:
            fine_idx = gt_classes < scores.shape[1]
            score_fine = scores[fine_idx]
            gt_classes_fine = gt_classes[fine_idx]
            #loss_cls = self.sigmoid_cross_entropy_loss(scores, gt_classes)
            loss_cls = self.sigmoid_cross_entropy_loss(score_fine, gt_classes_fine)
        else:
